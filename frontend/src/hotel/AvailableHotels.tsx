      ...
      <div className="overflow-auto h-full pb-4">
        {hotels.length < 1 ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {hotels.map((hotel) => (
              <div
                onClick={() => navigate(`/hotelDetails/${hotel.id}/${filters.start_date}/${filters.end_date}/${hotel.available_room_ids}`)}
                key={hotel.id}
                className="bg-white shadow-md rounded-2xl p-6 hover:shadow-xl transition-shadow duration-300 cursor-pointer">
